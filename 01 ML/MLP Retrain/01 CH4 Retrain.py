...
for rs in [120]:
    ...
    model = 'M3 MLP'
    mlp = Sequential()
    mlp.add(Dense(units = 60, activation = 'relu', input_dim = X.shape[1]))
    mlp.add(Dense(units = 60, activation = 'relu'))
    ...
    ch4_hist = mlp.fit(X_train_scaled, y_train, epochs = 1000, batch_size = 32)
    # Saving SSA model
    ...
    train_array.to_excel('{} RS{} train.xlsx'.format(model, rs))
    test_array.to_excel('{} RS{} test.xlsx'.format(model, rs))       
